...
typedef struct {
    ...
    TipoArquivo tipoArquivo; // Tipo de conteúdo
    time_t criado;
    time_t modificado;
    time_t acessado;
    int id;
...
No* criarArquivo(const char *nome, int tamanho, TipoArquivo tipo, int id, int permissao) {
    ...
    novo->arquivo.permissao = time(NULL);
    novo->arquivo.id = id;
        ...
        if (strncmp(comando, "exit", 4) == 0) break;
            ...
            while (aux != NULL) {
                if (aux->tipo == TIPO_ARQUIVO && strcmp(aux->arquivo.nome, nome) == 0) {
                    Arquivo *a = &aux->arquivo;
                    printf("Arquivo: %s\nTamanho: %d bytes\nTipo: %s\nID: %d\nPermissão: %d\n",
                        a->nome, a->tamanho, tipoArquivoStr(a->tipoArquivo), a->id, a->permissao);
                    encontrado = 1;
                    break;
                }
                aux = aux->proximo;
            ...
            printf("Comando inválido. Use mkdir, touch, ls, cd, cat, exit\n");
        }
    }
}
